...
graph TD
    ...
    E --> G[main branch protection: CI status + ruff + mypy]
...
| A grouped PR (`uv-minor-patch` / `actions-minor-patch`) mixes bump levels across its member dependencies | **Rejected design**: `dependabot/fetch-metadata`'s top-level `update-type` and `dependency-type` outputs are each computed independently across the *whole* PR (highest semver level; highest-priority dependency type) — they do not describe the same dependency, so a group containing (say) an indirect minor bump alongside a direct-dev patch bump would read as `update-type: minor` + `dependency-type: direct:development` and wrongly pass a single combined check. **Actual design**: the `eligibility` step evaluates `updated-dependencies-json`, which carries one entry per updated dependency, and requires (via `jq`'s `all(.[]; ...)`) that *every* entry independently satisfies patch-any-or-minor-dev — so one ineligible member anywhere in the group blocks the whole PR's auto-merge. | Working as intended; this is REQ-003/REQ-004's actual enforcement mechanism, not a residual gap. Covered by VLD-007 below. |
