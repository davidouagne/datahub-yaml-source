...
| `DOCUMENT` MCPs need a recent-enough GMS | Rejected outright on an older target deployment | Documented limitation, not a release blocker: the connector emits the MCPs correctly; whether an older target GMS accepts them is a property of that deployment, verified there at install time. |
| `schemaField` annotations need those URNs indexed by the target GMS | Column tags may not surface in the UI even though the MCPs succeed | Documented limitation, not a release blocker: the MCPs are correct; UI surfacing depends on the target GMS indexing `schemaField` entities, verified there at install time. |
...
| 2 | Is the target GMS recent enough for `DOCUMENT`, `applications`, and `displayProperties`? | AP-HP platform team | **Documented limitation** (not a 0.1.0 blocker) — the connector emits these aspects correctly; acceptance is a property of the target deployment. Recorded in Known Limitations and in `yaml.md` ("Target GMS compatibility"); end-to-end behaviour is exercised against a current local GMS in the release checklist. Confirmation against the AP-HP deployment stays a platform-team post-install check. |
| 3 | Does the target deployment index `schemaField` entities? | AP-HP platform team | **Documented limitation** (not a 0.1.0 blocker) — same framing as #2: the MCPs are correct, UI surfacing depends on the target GMS. Recorded in Known Limitations and in `yaml.md`. |
...
| 6 | `APPLICATION` has no lineage field: `application.yml` entries can't declare Consumes/Produces against `API`/`DATASET` entities (`applicationLineage` aspect isn't wired to any mixin). Should this be added? | maintainer | **Resolved** — implemented as `APPLICATION.applicationLineage.{consumes,produces}`, each a list of `{api: <id>}` or `{dataset: <DatasetRef>}` edges (exactly one of the two per edge, enforced with a `ValueError` at build time — same "malformed doc fails that document, not the whole run" convention as `DOCUMENT`'s native/external check). Maps to the `applicationLineage` aspect's `inputEdges`/`outputEdges` (`Consumes`/`Produces` relationships); no existence check against a `ReferenceIndex`, consistent with how `DATASET`/`API` refs are already treated as resolvable-but-unverified elsewhere (`upstreamLineage`, `SERVICE.apis`, `AGENT_SKILL.requiredTools`). See `build_application()` in `builders/application.py`. |
| 7 | `API` has no field to reference the `DATASET`(s) it reads from or returns (e.g. an APISQL endpoint and the query-result dataset it wraps). Should one be added? | maintainer | **Open** — same session as #6. Today the only link between an `API` and its backing `DATASET` is name-matching in free-text `description`/`externalUrl`, not a resolvable URN edge. Consuming repos should keep using the description-based convention until this lands — don't hand-write a not-yet-supported field into YAML in the meantime, it'll just fail ingestion. |
